...
"""Create Met-TL input data from incomplete methylation profiles.
...
``trans_train.py`` model evaluation using ``trans_eval.py``.
...
def extract_seq_windows(seq, pos, wlen, seq_index=1, assert_cpg=False):
    ...
    for i in range(nb_win):
        ...
        if seq[p:p + 2] != 'CG' and seq[p - 1:p + 1] != 'CG':
...
def map_values(values, pos, target_pos, dtype=None, nan=dat.CPG_NAN):
    ...
    idx = np.in1d(pos, target_pos) # Tests whether each element of a 1-D array is also present in a second array, returns boolean array.
    pos = pos[idx]
    values = values[idx]
    if not dtype:
        dtype = values.dtype
            ...
            for chunk in range(nb_chunk):
                ...
                if 'cpg' in chunk_outputs:
                    for name, value in six.iteritems(chunk_outputs['cpg']):
                        ...
                        # Round continuous values, leading to binary target for later prediction
