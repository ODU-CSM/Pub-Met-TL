...
"""Evaluate the prediction performance of a Met-TL model.
...
from transfer import evaluation as ev
from transfer import models as mod
from transfer.data import hdf
from transfer.utils import ProgressBar, to_list
...
class App(object):
    ...
    def create_parser(self, name):
        ...
        p.add_argument(
            '--loss_fn',
            type=str,
            help='name of the loss funtion - to get label distribution if it is kl_divergence',
    ...
    def main(self, name, opts):
        ...
        if opts.loss_fn is not None:
            loss_fn = opts.loss_fn
        else:
            loss_fn = None

        data_reader = mod.data_reader_from_model(
            model, replicate_names, replicate_names=replicate_names, loss_fn=loss_fn)
